...
matchStart.replaceChildren();
	if(selectIndex==2){
		for( let i=0; i < eval(answer[1]).length; i++){
			matchQuestion.innerText=questionLIst[selectIndex];
			let option = createElement('li', {prop:{className:'option', style:'float:left; margin-right:20px'}});
			let button = createElement('button', {prop:{ className:"button primary small", innerText:eval(answer[1])[i],id:option}});
			let br = createElement('br');
			option.prepend(buttom);
			option.prepend(br);
			option.addEventListener('click', ev =>{
				answer.push(button.innerText);
			})
